...
| `ATOM_PKI_ENROLLMENT_HTTP_HEADER_TIMEOUT_SECS` / `ATOM_PKI_ENROLLMENT_REQUEST_BODY_TIMEOUT_SECS`                             | `10` / `30`                                          | Bound header reads and the total request-body/handler lifetime before enrollment authentication |
...
| `ATOM_EVENTS_AMQP_URL`                                                                                                         | *(unset)*                                            | AMQP broker URL (e.g. `amqp://guest:guest@localhost:5672/%2f`); enables event outbox publishing when set |
| `ATOM_EVENTS_AMQP_EXCHANGE`                                                                                                    | *(empty)*                                            | Custom AMQP exchange name; leave empty to publish to default exchange                   |
...
| `ATOM_LOG_LEVEL`                                                                                                               | `info`                                               | Tracing log filter; accepts values such as `debug` or full env-filter directives        |
| `ATOM_LOG_FORMAT`                                                                                                              | `text`                                               | Log output format: `text` or structured `json`                                          |
