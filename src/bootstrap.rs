...
/// owner for a case-insensitively duplicated human email.
///
/// This runs after the pool is created but before `sqlx::migrate!().run()`. It
/// is deliberately read-only. Fresh databases have no legacy tables yet, and
/// databases that have already applied migration 025 already enforce the
/// invariant, so both cases are no-ops.
pub async fn preflight_legacy_email_uniqueness(pool: &PgPool) -> Result<()> {
    let legacy_tables_exist: bool = sqlx::query_scalar(
        r#"SELECT to_regclass('entities') IS NOT NULL
                  AND to_regclass('entity_emails') IS NOT NULL
                  AND to_regclass('_sqlx_migrations') IS NOT NULL"#,
    )
    .fetch_one(pool)
    .await
    .context("failed to inspect the database before the v1 email migration")?;
    if !legacy_tables_exist {
        return Ok(());
    }

    let migration_applied: bool = sqlx::query_scalar(
        "SELECT EXISTS (SELECT 1 FROM _sqlx_migrations WHERE version = 25 AND success)",
    )
    .fetch_one(pool)
    .await
    .context("failed to inspect migration 025 state")?;
    if migration_applied {
        return Ok(());
    }

    // This models the active rows migration 025 would see after its legacy
    // attribute backfill. Existing canonical rows are kept as-is; an entity
    // attribute contributes only when that entity has no canonical row, just
    // like the migration's LEFT JOIN predicate.
    let collisions = sqlx::query(
        r#"WITH prospective_active_emails AS (
    ...
    .fetch_all(pool)
    .await
    .context("failed to check legacy email uniqueness before migration 025")?;
    ...
    bail!(
        "v1 upgrade blocked before migration 025: case-insensitive legacy email \
         collision(s) found: {details}. Choose one owning entity for each email \
         and change or remove the other legacy email values in v0.50, then \
         restart Atom. This preflight did not modify the database"
    )
}

/// Refuse a v0.50 -> v1 migration when migration 007 would erase an
/// operator-defined action applicability row.
///
/// Migration 007 predates the v1 freeze and must remain byte-for-byte
/// immutable for databases that have already recorded its checksum. Its SQL
/// removes every applicability row for the two product-specific resource
/// types, including the three rows seeded by migration 001. This read-only
/// preflight makes the supported v0.50 maintenance upgrade safe: every row
/// must have an exact declaration in `ATOM_BOOTSTRAP_FILE`. Bootstrap then
/// restores the declarations after migrations, before the service accepts
/// traffic.
pub async fn preflight_product_applicability(
    pool: &PgPool,
    bootstrap: Option<&BootstrapConfig>,
) -> Result<()> {
    let legacy_tables_exist: bool = sqlx::query_scalar(
        r#"SELECT to_regclass('actions') IS NOT NULL
                  AND to_regclass('action_applicability') IS NOT NULL
                  AND to_regclass('_sqlx_migrations') IS NOT NULL"#,
    )
    .fetch_one(pool)
    .await
    .context("failed to inspect the database before migration 007")?;
    if !legacy_tables_exist {
        return Ok(());
    }

    let migration_applied: bool = sqlx::query_scalar(
        "SELECT EXISTS (SELECT 1 FROM _sqlx_migrations WHERE version = 7 AND success)",
    )
    .fetch_one(pool)
    .await
    .context("failed to inspect migration 007 state")?;
    if migration_applied {
        return Ok(());
    }

    let affected = sqlx::query(
        r#"SELECT actions.name, applicability.object_kind, applicability.object_type
           ...
           ORDER BY actions.name, applicability.object_kind, applicability.object_type"#,
    )
    .fetch_all(pool)
    .await
    .context("failed to check custom action applicability before migration 007")?;
    let declared = bootstrap
        .into_iter()
        .flat_map(|config| &config.capabilities)
        .flat_map(|capability| {
            capability.applicability.iter().map(|applicability| {
                (
                    capability.name.trim().to_string(),
                    applicability.object_kind.as_str().to_string(),
                    applicability.object_type.clone(),
                )
            })
        })
        .collect::<HashSet<_>>();
    let missing = affected
        .iter()
        .map(|row| -> Result<(String, String, Option<String>)> {
            Ok((
                row.try_get("name")?,
                row.try_get("object_kind")?,
    ...
    let details = missing
        .iter()
        .map(|(name, object_kind, object_type)| {
            format!(
                "{name} on {object_kind} / {}",
    ...
    bail!(
        "v1 upgrade blocked before migration 007: action applicability row(s) would be \
         removed without an exact bootstrap replacement: {details}. Declare every row \
         under capabilities[].applicability in ATOM_BOOTSTRAP_FILE and restart Atom. \
         Bootstrap will restore them after migration 007 and before traffic starts. This \
         preflight did not modify the database"
    )
}
...
async fn ensure_tenant(tx: &mut DbTransaction<'_>, tenant: &BootstrapTenant) -> Result<()> {
...
async fn ensure_entity(tx: &mut DbTransaction<'_>, entity: &BootstrapEntity) -> Result<()> {
...
async fn ensure_group(tx: &mut DbTransaction<'_>, group: &BootstrapGroup) -> Result<()> {
...
async fn ensure_resource(tx: &mut DbTransaction<'_>, resource: &BootstrapResource) -> Result<()> {
