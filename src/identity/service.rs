...
async fn finish_password_reset_in_tx(
    ...
    // who requested a change to their own mailbox could wait out the owner's
    // password recovery, then confirm — re-pointing password recovery at the
...
pub async fn request_email_change(
    ...
    let mut tx = pool.begin().await.map_err(db_err)?;
    // Serialize issuance on the entity lock — the same lock, taken in the
    // same order, that `confirm_email_change` and every other identity
    // mutation uses. Without it, two overlapping requests each supersede
    // only what was already committed, miss each other's uncommitted insert,
    // and both land live: two honourable pending tokens, with either
    // proposed email confirmable — the opposite of "latest request only".
    if super::repo::lock_active_entity(&mut tx, auth.entity_id)
        .await?
        .is_none()
    {
        return Err(AppError::not_found("account not found"));
    }
    // A config-managed identity belongs to the bootstrap YAML; applying a
    // self-service email change to it would desync the database from the
    // file and break the next startup's reconciliation. The confirm
    // transaction enforces the same guard — checked here too so a dead-end
    // token is never minted and mailed at all.
    crate::managed_by::ensure_not_config_managed_in_tx(&mut tx, "entities", auth.entity_id).await?;
...
pub async fn confirm_email_change(
    ...
    // The authoritative ownership guard — the same check the request path
    // applies. The entity row is already locked above, so the marker cannot
    // be stamped by a concurrent bootstrap reconciliation between the check
    // and the writes it protects.
    crate::managed_by::ensure_not_config_managed_in_tx(&mut tx, "entities", entity_id).await?;
    ...
    // Defensive re-sweep: consume every other still-pending token for this
    // entity, so only one email change can ever land from a confirm. Issuance
