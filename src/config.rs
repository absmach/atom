...
/// login clients see byte-identical behavior until a deployment opts in;
/// `access_token_expiry_secs` defaults to `JWT_EXPIRY_SECS` unchanged.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
fn refresh_tokens_from_env(jwt_expiry_secs: u64, kek_present: bool) -> Result<RefreshTokenConfig> {
    ...
    // Only enforced when the feature is actually on: while disabled, the two
    // lifetimes fall back to independent defaults that can legitimately
    // collide (e.g. JWT_EXPIRY_SECS=2592000) without meaning anything.
    if cfg.enabled {
...
fn purge_from_env() -> Result<PurgeConfig> {
    ...
    // Validated unconditionally, unlike `retention_days`: `spawn_refresh_token_cleanup`
    // reads these same two knobs whenever refresh tokens are enabled,
    // independently of ATOM_PURGE_ENABLED, so a dormant invalid value could
    // still reach that job and panic or error on every tick.
...
mod tests {
    ...
    /// Regression test: the ordering check must never run while the feature
    /// is disabled, or a long `JWT_EXPIRY_SECS` alone could collide with the
    /// unused default refresh lifetime and refuse to start.
    #[test]
    fn refresh_tokens_disabled_ignores_colliding_default_lifetimes() {
        let _guard = ENV_LOCK.lock().expect("env lock");
        clear_hardening_env();
        let _db_guard = DatabaseUrlGuard::set();

        std::env::set_var("JWT_EXPIRY_SECS", "2592000");
        let cfg = Config::from_env().expect("disabled refresh tokens must not block startup");
        assert!(!cfg.refresh_tokens.enabled);
        assert_eq!(
    ...
    /// Regression test: `spawn_refresh_token_cleanup` reads `purge.interval_secs`/
    /// `purge.batch_size` whenever refresh tokens are enabled, independently
    /// of `ATOM_PURGE_ENABLED`, so those knobs must be validated even while
    /// purge itself is disabled.
    #[test]
    fn purge_interval_and_batch_are_validated_even_when_purge_is_disabled() {
        let _guard = ENV_LOCK.lock().expect("env lock");
        clear_hardening_env();
        let _db_guard = DatabaseUrlGuard::set();

        assert!(!Config::from_env().expect("config").purge.enabled);

        std::env::set_var("ATOM_PURGE_INTERVAL_SECS", "0");
        let err = Config::from_env().expect_err("zero interval must fail even while disabled");
        assert!(err.to_string().contains("ATOM_PURGE_INTERVAL_SECS"));
        std::env::remove_var("ATOM_PURGE_INTERVAL_SECS");

        std::env::set_var("ATOM_PURGE_BATCH_SIZE", "0");
        let err = Config::from_env().expect_err("zero batch size must fail even while disabled");
        assert!(err.to_string().contains("ATOM_PURGE_BATCH_SIZE"));
    ...
    fn clear_hardening_env() {
        for name in [
            ...
            "ATOM_ACCESS_TOKEN_EXPIRY_SECS",
            "ATOM_PURGE_ENABLED",
            "ATOM_PURGE_RETENTION_DAYS",
            "ATOM_PURGE_INTERVAL_SECS",
            "ATOM_PURGE_BATCH_SIZE",
            "ATOM_LOG_LEVEL",
            "ATOM_LOG_FORMAT",
