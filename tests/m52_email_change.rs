...
/// P1 regression: a password reset is a full identity-state reset, so a
/// pending email-change request minted before it (possibly by a briefly
/// compromised session) must not survive it — otherwise the attacker who
/// requested the change waits out the owner's recovery, then confirms and
/// re-points password recovery at their own mailbox.
#[tokio::test]
#[ignore]
async fn password_reset_consumes_pending_email_change_tokens() {
    let pool = common::pool().await;
    let (entity_id, session_id, old_email) = human_with_session(&pool).await;

    // One pending request minted through the real request path.
    service::request_email_change(
        &pool,
        &Config::for_tests(),
        &session_auth(entity_id, session_id),
        EmailChangeRequest {
            new_email: format!("hijack-1-{entity_id}@example.test"),
        },
    )
    .await
    .expect("request email change");
    // And one with a known plaintext, so the post-reset confirm attempt can
    // be driven end to end.
    let doomed = mint_confirm_token(
        &pool,
        entity_id,
        session_id,
        &old_email,
        &format!("hijack-2-{entity_id}@example.test"),
        Utc::now() + Duration::minutes(30),
    )
    .await;
    assert_eq!(pending_token_count(&pool, entity_id).await, 2);
    ...
    assert_eq!(
        pending_token_count(&pool, entity_id).await,
        0,
        "password recovery must consume every pending email-change request"
    );

    // The full takeover path is closed: a pre-reset token can no longer be
    // confirmed even though the current email never drifted.
    let err = service::confirm_email_change(
        &pool,
        &Config::for_tests(),
        None,
        false,
        EmailChangeConfirmRequest { token: doomed },
    )
    .await
...
/// P2 regression: a bootstrap-provisioned (`managed_by = 'config'`) global
/// human must not move its email through the self-service flow — the YAML
/// is the source of truth, and a drifted row breaks the next startup's
/// reconciliation. Both the request (fail fast, never mint/mail a dead-end
/// token) and the confirm (the authoritative gate) must reject it.
#[tokio::test]
#[ignore]
async fn email_change_rejects_config_managed_identities() {
    ...
    .await
    .expect_err("request must reject a config-managed identity");
    assert!(matches!(err, AppError::Conflict(_)), "{err:?}");
    assert_eq!(
        pending_token_count(&pool, entity_id).await,
        0,
        "no token may be minted for a config-managed identity"
    );

    // A token minted before the row was stamped (or by an older deployment)
    // must not apply the change at confirm time either.
    let token = mint_confirm_token(
        &pool,
        entity_id,
        session_id,
        &old_email,
        &format!("new-{entity_id}@example.test"),
        Utc::now() + Duration::minutes(30),
    )
    .await;
    let err = service::confirm_email_change(
...
/// P2 regression: issuance must serialize on the entity lock, or two
/// overlapping requests each supersede only what was already committed,
/// miss each other's uncommitted insert, and leave two honourable pending
/// tokens — violating the documented "latest request only" behaviour.
/// The lock-holder forces the race window wide open: pre-serialization
/// requests (which never touched the entities row) would complete
/// underneath the holder and land both tokens.
#[tokio::test]
#[ignore]
async fn concurrent_requests_serialize_on_the_entity_lock_and_leave_one_pending_token() {
    let pool = common::pool().await;
    ...
    let target_a = format!("left-{entity_id}@example.test");
    ...
    let auth_b = session_auth(entity_id, session_id);
    let target_b = format!("right-{entity_id}@example.test");
    ...
    // Both requests must block on the held lock — proving the request path
    // takes the entity lock at all (the serialization point).
    assert!(
        tokio::time::timeout(std::time::Duration::from_millis(300), &mut a)
            .await
            .is_err(),
        "request a must block on the entity lock"
    );
    assert!(
        tokio::time::timeout(std::time::Duration::from_millis(300), &mut b)
            .await
            .is_err(),
        "request b must block on the entity lock"
    );
    holder.rollback().await.expect("release the entity lock");

    let (a, b) = (
        tokio::time::timeout(std::time::Duration::from_secs(5), a)
            .await
            .expect("request a should finish once the lock is released")
            .expect("request a task"),
        tokio::time::timeout(std::time::Duration::from_secs(5), b)
            .await
            .expect("request b should finish once the lock is released")
    ...
    assert_eq!(
        pending_token_count(&pool, entity_id).await,
        1,
        "serialized issuance must leave exactly one honourable pending token"
    );
}
