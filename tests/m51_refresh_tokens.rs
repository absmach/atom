...
async fn exchange_rotates_the_token_and_works_after_access_jwt_expiry() {
    ...
    let pair =
        identity_service::exchange_refresh_token(&pool, &cfg, &signer, None, &original_refresh)
            .await
...
async fn reuse_of_a_consumed_token_revokes_the_session_and_its_live_descendant() {
    ...
    let descendant_dead =
        identity_service::exchange_refresh_token(&pool, &cfg, &signer, None, &pair.refresh_token)
            .await
...
async fn concurrent_exchange_of_the_same_token_yields_exactly_one_success() {
    ...
    let (first, second) = tokio::join!(
        identity_service::exchange_refresh_token(&pool, &cfg, &signer_a, None, &refresh_token),
        identity_service::exchange_refresh_token(&pool, &cfg, &signer_b, None, &refresh_token),
    );
...
/// Regression test: an entity whose *tenant* (not the entity itself) is
/// inactive must get the same generic error as every other rejection
/// reason. `lock_active_entity` returns `Err(NotFound)` for this case
/// specifically, rather than `Ok(None)`, and that must not leak through.
#[tokio::test]
#[ignore]
async fn inactive_tenant_rejects_exchange_with_the_generic_error() {
    let pool = common::pool().await;
    let keys = active_keys(&pool).await;
    let cfg = refresh_enabled_config(3600, 7200);
    let signer = JwtSigner::from_key(&keys.primary).expect("signer");
    ...
    let err = identity_service::exchange_refresh_token(
        ...
        &signer,
        None,
        &login_response.refresh_token.expect("refresh token"),
    )
    .await
    .expect_err("inactive tenant must reject exchange");
...
/// Regression test: the deprecated `refreshSession` mutation must never
/// shorten a refresh-enabled session's long family deadline down to
/// `jwt_expiry_secs` — doing so would silently kill every future refresh
/// exchange for that session once the (much shorter) JWT-based expiry
/// passes, even though the family itself remains days from expiring.
#[tokio::test]
#[ignore]
async fn refresh_session_never_truncates_a_refresh_enabled_session() {
    let pool = common::pool().await;
    let keys = active_keys(&pool).await;
    let cfg = refresh_enabled_config(3600, 7200);
    let signer = JwtSigner::from_key(&keys.primary).expect("signer");
    ...
    // The deprecated mutation only knows `cfg.jwt_expiry_secs` (3600s here),
    // far shorter than the 7200s family deadline just established at login.
    ...
    // The refresh-token family itself must still work afterward.
    let pair = identity_service::exchange_refresh_token(
        &pool,
        &cfg,
        &signer,
        None,
