...
Deliver a production-ready, OpenBao-free, multi-tenant private PKI inside Atom. The production root remains offline; each tenant receives a versioned intermediate CA; Atom remains the certificate credential and authorization source of truth.
      ...
      PR-007 Renewal  PR-008 Revocation
            ...
            +------+------+
              ...
              PR-010 OCSP
                   |
                   v
        PR-011 Runtime Resolver v2
                   |
                   v
        PR-012 Magistrala Integration
                   |
                   v
...
Includes PR-001 through PR-004.

Exit gate:

- authority hierarchy and lifecycle are persisted;
- existing v1 certificate behavior remains unchanged;
- no plaintext CA key can be persisted;
- a signer interface exists;
- profiles are canonical and tenant-independent request input cannot override them;
...
- a tenant intermediate can be imported or provisioned;
- CSR and generated-key leaf issuance derive issuer from the target entity tenant;
...
- No standalone `absmach/certs` source of truth.
...
- Issuer is derived from the target entity tenant.
- No plaintext private keys in logs, database, events, traces, or errors.
...
- A tenant certificate must never reference another tenant's issuer.
...
3. New tenant issuance starts only after issuer-aware service paths are merged.
...
6. Hard tenant purge may remove tenant authority material only after the product retention policy permits purge.

## Required review gates

Security review is mandatory for PR-002, PR-003, PR-005, PR-006, PR-010, PR-011, and PR-013.

Database review is mandatory for any PR changing authority, credential, CRL, OCSP, migration, purge, or uniqueness semantics.

Magistrala integration review is mandatory for PR-011 and PR-012.

## Completion

The project is complete only when every PR acceptance criterion is met, all CI checks pass, migration and rollback behavior are documented, and the production runbook covers root custody, issuer rotation, backup, restore, compromise, and tenant deletion.