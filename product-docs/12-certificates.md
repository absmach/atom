...
Rate limits: `ATOM_PKI_ENROLLMENT_ENTITY_RATE_LIMIT`, `_TENANT_RATE_LIMIT`, `_ENTITY_RATE_WINDOW_SECS`, `_TENANT_RATE_WINDOW_SECS`, plus the normal IP-based public-route limiter. TLS/HTTP connection bounds: `_MAX_CONNECTIONS`, `_MAX_CONNECTIONS_PER_IP`, `_HANDSHAKE_TIMEOUT_SECS`, `_HTTP_HEADER_TIMEOUT_SECS`, `_REQUEST_BODY_TIMEOUT_SECS`, `_CONNECTION_TIMEOUT_SECS`, `_SHUTDOWN_DRAIN_TIMEOUT_SECS`.
...
| `ATOM_PKI_ENROLLMENT_HTTP_HEADER_TIMEOUT_SECS` / `_REQUEST_BODY_TIMEOUT_SECS` | `10` / `30` | Maximum time to receive HTTP headers and a request body. |
