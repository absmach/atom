...
| `make release`              | Builds both images from a clean exact `vX.Y.Z` tag using that versioned image tag.      |
...
`ghcr.io/absmach/atom-ui:v0.50.0` in the local image store. `make release`
does not push; pushing the Git tag triggers the image workflow, which runs the
Rust and Frontend suites first and then publishes both the versioned tags and
`:latest`. `make latest` uses `git describe --tags --always --dirty` for the
embedded runtime version while keeping the mutable `latest` image tag.
