...
does not push; pushing the Git tag triggers the image workflow. `make latest`
uses `git describe --tags --always --dirty` for the embedded runtime version
while keeping the mutable `latest` image tag.
