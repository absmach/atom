...
When upgrading an existing checkout, `make up` automatically updates the two
legacy demo paths in `.env` to their current locations under `config/demo/`.
Custom paths are never changed.
...
| `make latest`               | Builds both `:latest` images with Git-derived version and revision metadata.           |
...
For a versioned release, tag the exact clean commit and build both images:
...
The result is `ghcr.io/absmach/atom:v1.0.0` and
`ghcr.io/absmach/atom-ui:v1.0.0` in the local image store, with the local
`:latest` tags moved onto the same images so `make up` runs what was just
released. `make release` does not push; pushing the Git tag triggers the image
workflow, which runs the Rust and Frontend suites first and then publishes both
the versioned tags and `:latest`. Publishing is serialized across branch and
tag events, and CI enforces the same exact `vX.Y.Z` guard as `make release`.
`make latest` uses `git describe --tags --always --dirty` for the embedded
runtime version while keeping the mutable `latest` image tag.
