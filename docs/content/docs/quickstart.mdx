...
the versioned tags and `:latest`. `make latest` uses
`git describe --tags --always --dirty` for the embedded runtime version while
keeping the mutable `latest` image tag.
