...
management therefore use the same deny-overrides-allow PDP as other protected
objects: `read` or `manage` controls visibility, `manage` controls updates and
status changes, and `execute` or `manage` controls invocation. Tenant and
platform scopes inherit normally; exact and `object_kind = api_endpoint`
grants provide least-privilege administration. List queries apply these grants
before pagination, so an exact deny cannot leak an endpoint through a broader
tenant listing.
