...
| `pki_authorities` | Managed roots, platform intermediates, platform leaf issuers, and tenant intermediates. |
| `certificate_profiles` | Reusable leaf issuance and renewal policy. |
| `certificate_revocations` | Immutable revocation evidence for exact certificate credentials. |
| `certificate_crl_state` | Cached CRL state for each managed issuer. |
...
`tenant`, `role`, `policy`, `credential`, `api_endpoint`, `audit_log`, and
`signing_key`.
...
Exact-object scopes store a UUID, so every UUID-addressable protected object
shares one database-enforced namespace. The registry covers tenants, entities,
resources, principal and object groups, roles, credentials, direct policies,
role assignments, and custom API endpoints. Soft-deleted objects retain their
reservation until physical purge. This prevents an exact grant for one kind
from matching a different kind that happened to reuse the same UUID. Profiles
and individual audit-log rows are not exact-object targets; signing keys use a
textual `kid` and stay platform-scoped.

## Certificate State

Atom stores every managed authority certificate and its lifecycle state in
Postgres. Managed CA private keys are envelope-encrypted in Postgres by default;
an authority can instead store an opaque reference to a non-exportable PKCS#11
key. The production root private key remains offline and is never accepted by
Atom.

Postgres stores certificate lifecycle state including:

- authority rows in `pki_authorities`;
- issued certificate rows in `credentials`;
- immutable revocation rows in `certificate_revocations`;
- cached per-issuer CRL bytes and CRL numbers in `certificate_crl_state`.
