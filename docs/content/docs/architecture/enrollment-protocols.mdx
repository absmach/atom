---
title: EST and ACME
description: Why Atom uses EST for managed device enrollment instead of ACME.
---
...
[`src/certs/enrollment/`](https://github.com/absmach/atom/tree/main/src/certs/enrollment) or before
...
- Server implementation: [`src/certs/enrollment/est.rs`](https://github.com/absmach/atom/blob/main/src/certs/enrollment/est.rs) — the five RFC 7030 endpoints, wire encoding only.
- Business logic (subject, scope, profile, issuer, lifecycle, rate-limit, audit): [`src/certs/enrollment/service.rs`](https://github.com/absmach/atom/blob/main/src/certs/enrollment/service.rs).
- Dedicated TLS listener for enrollment: [`src/certs/enrollment/tls.rs`](https://github.com/absmach/atom/blob/main/src/certs/enrollment/tls.rs).
- Interop test using an independent EST client: [`tests/m41_pki_est.rs`](https://github.com/absmach/atom/blob/main/tests/m41_pki_est.rs).
- Historical per-PR design notes are retained in `docs/archive/product-plans`.
...
write. See the "EST client" section of [PKI Release Testing](../development/pki-release-testing)
