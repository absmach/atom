...
- Authorized listing (`authorizedObjectIds` and the entity/resource/group list
  queries) is **ceiling-aware**: a scoped token listing its own authorized set gets
  `owner live grants ∩ ceiling`, with correct paging and totals. Like the coarse
  gates, the listing cannot evaluate per-request ABAC context, so **conditional
  ceiling entries fail closed** there — an object admitted only by a conditional
  entry appears via per-object `authzCheck`, not in listings. Tenant *visibility*
  listing (`tenants`) is not ceiling-aware and still fails closed for scoped tokens.
