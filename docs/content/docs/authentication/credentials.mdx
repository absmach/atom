...
<Callout type="warn">
The dedicated `createApiKey` mutation has been removed. Token creation now goes
through a single surface — [`createAccessToken`](#scoped-access-tokens) — which
always produces a scoped token capped by a permission ceiling. Unscoped `access_token`
credentials still authenticate (bootstrap and pre-existing keys), but new ones are
no longer minted through the API. A "mirror owner" ceiling preset that reproduces the
unscoped, live-grant-tracking behaviour under the single primitive is planned.
</Callout>

For provisioned service access today, mint a scoped token for the service entity
(delegated `createAccessToken` with `subjectId`) scoped to exactly what the service
needs. The full token is returned **exactly once** and cannot be recovered.
