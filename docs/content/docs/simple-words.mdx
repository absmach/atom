...
| Assignment Guardrail | A safety rule for what access may be assigned | Devices may publish to channels but may not manage them |
...
Assignment Guardrails are another safety check. They do not answer whether a
live request is allowed. They answer whether Atom should let an administrator
create access state, such as giving a device a role that can `manage` channels.
...
Atom stores every managed authority in Postgres. CA keys are envelope-encrypted
there by default or kept as non-exportable objects in a PKCS#11 HSM. The
production root private key stays offline and never enters Atom.
