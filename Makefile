...
# Migrate only the exact legacy demo paths generated by older Atom releases.
# LF and CRLF line endings are preserved; user-selected custom paths are untouched.
migrate-dev-env: $(DEV_ENV_FILE)
	@set -e; \
	tmp="$(DEV_ENV_FILE).atom-migrate"; \
	trap 'rm -f "$$tmp"' EXIT; \
	cp -p "$(DEV_ENV_FILE)" "$$tmp"; \
	cr="$$(printf '\r')"; \
	sed \
		-e "s|^ATOM_BOOTSTRAP_FILE=\./bootstrap\.yaml$${cr}$$|ATOM_BOOTSTRAP_FILE=./config/demo/bootstrap.yaml$${cr}|" \
		-e "s|^ATOM_CALLOUTS_FILE=\./callout\.yaml$${cr}$$|ATOM_CALLOUTS_FILE=./config/demo/callouts.yaml$${cr}|" \
		-e 's|^ATOM_BOOTSTRAP_FILE=\./bootstrap\.yaml$$|ATOM_BOOTSTRAP_FILE=./config/demo/bootstrap.yaml|' \
		-e 's|^ATOM_CALLOUTS_FILE=\./callout\.yaml$$|ATOM_CALLOUTS_FILE=./config/demo/callouts.yaml|' \
		"$(DEV_ENV_FILE)" > "$$tmp"; \
	if ! cmp -s "$(DEV_ENV_FILE)" "$$tmp"; then \
		mv "$$tmp" "$(DEV_ENV_FILE)"; \
		echo "==> updated legacy demo config paths in $(DEV_ENV_FILE)"; \
	fi; \
	trap - EXIT; \
	rm -f "$$tmp"
...
up: migrate-dev-env pki-material
