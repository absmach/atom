...
-- New for the launch baseline: rotating refresh tokens (issue #100). A
-- refresh token is an opaque, KEK-keyed-HMAC-verified credential bound to a
-- session, kept in its own table (not `credentials`) so consumed rows can be
-- retained until the family's absolute deadline for replay detection.
